...
`TakeutiGLC/Syntax/SelectedClosing.lean` now makes this distinction executable: a `VariableOccurrenceSelection` is consumed path-sensitively so that only the indicated occurrences are closed into the new de Bruijn block, while unindicated occurrences of the same free name remain free.
...
Indication becomes central in §5 substitution. `TakeutiGLC/Syntax/Occurrence.lean` implements it as finite auxiliary selections of structural occurrence paths, with separate variable and function selections and predicates for §3.3 full indication. `SelectedClosing.lean` is the first stable consumer of those paths. The stable raw syntax therefore still has no `indicated` constructor.
...
- arbitrary-height complete variable substitution uses hereditary block instantiation: positive-level formal variables are beta-reduced by recursively invoking a strictly lower-level instantiator, matching the induction pattern stated immediately after §5.2.26;
...
- the §5.2.29–§5.2.35 preservation/identity/commutation theorems, functional substitution, and the later interaction laws;
