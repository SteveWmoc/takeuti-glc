...
At a matching `freeVar`, the replacement is inserted directly and is not recursively substituted into itself, corresponding to §5.2.3. Special and bound variable occurrences are left untouched, while function applications and logical constructors recurse structurally. This is **complete** substitution in Takeuti's §5.2 sense: every occurrence of the target is replaced. The later §5.6 operation that substitutes only at indicated places is a distinct selection-aware layer. When the recursion crosses a variable quantifier, the replacement is weakened in the variable namespace; when it crosses a function quantifier, it is weakened in the function namespace; and when it enters a Takeuti abstraction block, it is weakened by the whole variable-block size.
